 ...
 */
public class LoRa_ReadSingleData extends Activity {
    EditText etJbid;
    Button btnEnter;
    ...
    void init() {
        setContentView(R.layout.lora_activity_readsingledata);
        this.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);//默认不弹出输入框
        etJbid = (EditText) findViewById(R.id.LoRa_ReadSingleData_et_jbid);
        btnEnter = (Button) findViewById(R.id.LoRa_ReadSingleData_btn_enter);
        btnEnter.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                if (HzyUtils.isEmpty(etJbid.getText().toString())){
                    HintDialog.ShowHintDialog(LoRa_ReadSingleData.this,"输入信息不可为空","提示");
                    return;
                }
                HzyUtils.showProgressDialog1(LoRa_ReadSingleData.this, "正在读取...");
                String jbid = etJbid.getText().toString().trim();
                while (jbid.length() < 14) {
                    jbid = "0" + jbid;
                }
                String sendMsg = "001600d4000807" + jbid;
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0x00:
                    ...
                    if (MenuActivity.METER_STYLE.equals("L")) {//LoRa表
                        ...
                        if (getMsg.contains("00160101") && getMsg.length() >= 40) {
                            HintDialog.ShowHintDialog(LoRa_ReadSingleData.this, "模块序号:" + getMsg.substring(8, 16)
                                    + "\n基表ID:" + getMsg.substring(16, 30)
                                    + "\n当前读数" + getMsg.substring(30, 34) + "." + getMsg.substring(34, 36), "替换成功");

                        } else if (getMsg.equals("00160008e026")) {
                            HintDialog.ShowHintDialog(LoRa_ReadSingleData.this, "表信息不存在", "成功");
                        }
                ...
                case 0x99:
                    HintDialog.ShowHintDialog(LoRa_ReadSingleData.this,"未收到返回信息","提示");
                    HzyUtils.closeProgressDialog();
                    break;
            }
        }
